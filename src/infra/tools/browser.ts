  ...
  return [
    {
      ...
      async execute(args) {
        // TODO: stage 4
        // 1. const url = String(args.url ?? "")
        // 2. const err = checkDomain(url, domains)
        // 3. if (err) return { success: false, message: err }
        // 4. return await browser.navigate(url)
        throw new Error("TODO: stage 4 — browser_navigate 未实现");
      },
    },

    {
      name: "browser_click",
      description: "点击页面中 index 对应的可交互元素（由 browser_navigate/browser_view 返回的 index）。",
      parameters: {
        index: { type: "integer", description: "元素索引（来自 interactive_elements 列表）" },
      },
      required: ["index"],
      async execute(args) {
        // TODO: stage 4
        // return await browser.click({ index: Number(args.index) })
        throw new Error("TODO: stage 4 — browser_click 未实现");
      },
    },

    {
      name: "browser_input",
      description: "在 index 对应的输入框里输入文本，press_enter 控制是否回车提交。",
      parameters: {
      ...
      async execute(args) {
        ...
        // })
        throw new Error("TODO: stage 4 — browser_input 未实现");
      },
    },
...

    {
      ...
      async execute() {
        ...
        try {
          ...
          return { success: true, message: "截图成功", data: { base64: buf.toString("base64") } };
        } catch (e) {
