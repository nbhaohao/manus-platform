...
export function sessionToRow(session: Session): Record<string, unknown> {
  ...
  return {
    ...
    title: session.title,
    unread_message_count: session.unreadMessageCount,
    latest_message: session.latestMessage,
    latest_message_at: session.latestMessageAt,
...
function rowToSession(row: Record<string, unknown>): Session {
  return new Session({
    ...
    latestMessageAt: row.latest_message_at
      ? new Date(row.latest_message_at as string)
      : null,
...
export class PgSessionRepository implements SessionRepository {
  ...
  async save(session: Session): Promise<void> {
    ...
    if (existing.length === 0) {
      await this.db.insert(sessions).values(row as any);
    } else {
      await this.db
        .update(sessions)
        .set(row as any)
  ...
  async getById(sessionId: string): Promise<Session | null> {
    ...
    return rows.length
      ? rowToSession(rows[0] as Record<string, unknown>)
      : null;
  }
  ...
  async incrementUnreadMessageCount(sessionId: string): Promise<void> {
    const rows = await this.db
      .select({ c: (sessions as any).unread_message_count })
      .from(sessions)
      .where(eq(sessions.id, sessionId));
  ...
  async decrementUnreadMessageCount(sessionId: string): Promise<void> {
    ...
    await this.db
      .update(sessions)
      .set({
        unread_message_count: Math.max(0, ((rows[0] as any).c ?? 0) - 1),
      } as any)
      .where(eq(sessions.id, sessionId));
  }
  ...
  async addEvent(sessionId: string, event: Event): Promise<void> {
    const rows = await this.db
      .select({ events: (sessions as any).events })
      .from(sessions)
      .where(eq(sessions.id, sessionId));
    if (!rows.length) return;
    ...
    await this.db
      .update(sessions)
      .set({ events: [...existing, eventData] } as any)
      .where(eq(sessions.id, sessionId));
  }

  async addFile(sessionId: string, file: File): Promise<void> {
    const rows = await this.db
      .select({ files: (sessions as any).files })
      .from(sessions)
      .where(eq(sessions.id, sessionId));
    if (!rows.length) return;
    ...
    await this.db
      .update(sessions)
      .set({ files: [...existing, file] } as any)
      .where(eq(sessions.id, sessionId));
  }
  ...
  async getFileByPath(
    ...
    const list = ((rows[0] as any).files ?? []) as Array<
  ...
  async saveMemory(
    ...
    const rows = await this.db
      .select({ memories: (sessions as any).memories })
      .from(sessions)
      .where(eq(sessions.id, sessionId));
    if (!rows.length) return;
    const existing = ((rows[0] as any).memories ?? {}) as Record<
      string,
      unknown
    >;
    const patch = {
      ...existing,
      [agentName]: { messages: memory.getMessages() },
    };
    await this.db
      .update(sessions)
      .set({ memories: patch } as any)
      .where(eq(sessions.id, sessionId));
  }
  ...
  async getMemory(sessionId: string, agentName: string): Promise<Memory> {
    const rows = await this.db
      ...
      .from(sessions)
      .where(eq(sessions.id, sessionId));
    const memories = (rows[0] as any)?.memories as
      | Record<string, { messages: LLMMessage[] }>
      | undefined;
