...
import { Session, SessionStatus, type SerializedMemory } from '../models/session.ts'
...
export interface SessionRepository {
  ...
  updateLatestMessage(sessionId: string, message: string, at: Date): Promise<void>
...
export class InMemorySessionRepository implements SessionRepository {
  ...
  async save(session: Session): Promise<void> {
    // TODO: stage 2 — this.db.set(session.id, session)
    throw new Error('TODO: stage 2 — InMemorySessionRepository.save')
  }

  async getById(sessionId: string): Promise<Session | null> {
    // TODO: stage 2 — return this.db.get(sessionId) ?? null
    throw new Error('TODO: stage 2 — InMemorySessionRepository.getById')
  }

  async saveMemory(sessionId: string, agentName: string, memory: Memory): Promise<void> {
    // TODO: stage 2 — 取/建 memDb[sessionId] Map，设 agentName → { messages: memory.getMessages() }
    throw new Error('TODO: stage 2 — InMemorySessionRepository.saveMemory')
  }

  async getMemory(sessionId: string, agentName: string): Promise<Memory> {
    // TODO: stage 2 — 从 memDb[sessionId]?.[agentName] 取 messages
    //   const data = this.memDb.get(sessionId)?.get(agentName)
    //   const mem = new Memory()
    //   data?.messages.forEach(m => mem.addMessage(m))
    //   return mem
    throw new Error('TODO: stage 2 — InMemorySessionRepository.getMemory')
  }

  // ── gen：其余 stub 方法（不测，仅满足 SessionRepository 接口）────────────
  async getAll(): Promise<Session[]> { return [...this.db.values()] }
  async deleteById(id: string): Promise<void> { this.db.delete(id) }
  async updateTitle(id: string, title: string): Promise<void> {
    const s = this.db.get(id); if (s) s.title = title
  }
  async updateLatestMessage(id: string, msg: string, at: Date): Promise<void> {
    const s = this.db.get(id)
    if (s) { s.latestMessage = msg; s.latestMessageAt = at }
  }
  async updateUnreadMessageCount(id: string, count: number): Promise<void> {
    const s = this.db.get(id); if (s) s.unreadMessageCount = count
  }
  async incrementUnreadMessageCount(id: string): Promise<void> {
    const s = this.db.get(id); if (s) s.unreadMessageCount++
  }
  ...
  async updateStatus(id: string, status: SessionStatus): Promise<void> {
    const s = this.db.get(id); if (s) s.status = status
  }
  async addEvent(id: string, event: Event): Promise<void> {
    const s = this.db.get(id); if (s) s.events.push(event)
  }
  async addFile(id: string, file: File): Promise<void> {
    const s = this.db.get(id); if (s) s.files.push(file)
  }
